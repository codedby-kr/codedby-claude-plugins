---
name: ask-claude-web
argument-hint: "claude.ai한테 [질문 내용]"
description: "chrome-devtools MCP로 claude.ai 웹 탭에 질문을 보내고 응답을 읽어오는 스킬. 입력 필드 찾기, 메시지 전송, 스트리밍 완료 감지(polling), 마지막 응답 추출의 evaluate_script 코드와 DOM 셀렉터 버전 이력 포함. 'claude.ai한테 물어봐', 'claude.ai한테 질문해', '웹 클로드한테 해결해달라고 해', '이건 claude.ai가 더 잘 알 거야' 같은 요청에 트리거. claude.ai와 자동 대화가 필요한 모든 상황에서 반드시 이 스킬을 로드할 것. take_snapshot은 절대 사용 금지(13만자 토큰 낭비)."
---

# ask-claude-web 스킬

## 개요
chrome-devtools MCP를 통해 Chrome에서 열린 claude.ai 탭에 질문을 보내고 응답을 읽어오는 스킬.
"claude.ai한테 물어봐", "웹 클로드한테 질문해" 등으로 지시하면 이 스킬을 사용한다.

## 절대 규칙

### 파일 첨부는 반드시 클립보드 방식을 사용하라
**파일 내용을 evaluate_script에 넣지 마라.** 파일 내용이 Claude Code 컨텍스트를 거치면 토큰이 낭비된다.
...
- evaluate_script의 args로 파일 내용을 전달하지 마라 (args는 uid 전용이라 작동도 안 함)
...
#### 주의사항
- **Windows:** `System.Windows.Forms.Clipboard` 사용 (PowerShell)
- **macOS:** `osascript` 사용 (실험적 — claude.ai Ctrl+V 붙여넣기 미검증)
- **Linux:** `xclip` 사용 (실험적 — claude.ai Ctrl+V 붙여넣기 미검증)
- 인자로 파일 경로를 여러 개 넣으면 한 번의 Ctrl+V로 모두 첨부됨
- 존재하지 않는 파일은 `[SKIP]`으로 건너뜀
- 파일 내용이 Claude Code 컨텍스트를 거치지 않으므로 대용량 파일도 부담 없음

### take_snapshot 사용 금지
claude.ai 페이지는 스냅샷이 13만자 이상이라 토큰을 낭비한다.
...
1. `select_page`로 claude.ai 탭 선택 (현재 열린 채팅창을 그대로 사용)
2. (파일 첨부 필요 시) clip-files 스크립트로 클립보드 복사 → 포커스 → `Control+v` / `Meta+v`
3. `evaluate_script`로 입력 필드 포커스 + 텍스트 입력
4. `press_key`("Enter")로 전송
5. `evaluate_script`로 스트리밍 완료 대기 (async Promise — 단일 호출)
5b. (권장) `evaluate_script`로 scroll-to-bottom (별도 호출)
6. `evaluate_script`로 응답 텍스트 추출
7. 추출한 텍스트를 바탕으로 작업 수행
...
유저가 다음과 같이 지시하면 이 스킬을 사용:
- "claude.ai한테 물어봐"
- "claude.ai한테 이거 해결해달라고 해"
- "웹 클로드한테 질문해"
- "이건 claude.ai가 더 잘 알 거야"

## 질문 작성 요령

claude.ai에 보내는 질문은 맥락이 충분해야 좋은 답이 온다:
- 현재 상황 설명 (뭘 하다가 문제가 생겼는지)
- 관련 코드 스니펫 포함
- 이미 시도한 것들 명시
- 원하는 결과 명확히 기술
...
### Chrome 권한 대화상자가 계속 뜸
→ 정상 동작. Allow를 누르면 됨. 매번 연결 시마다 뜰 수 있음.
...
| 버전 | 날짜 | 셀렉터/방법 | 상태 |
|------|------|------------|------|
| v1 | 2026-03-22 | `[contenteditable="true"][data-placeholder]` → `fieldset [contenteditable="true"]` → `[contenteditable="true"]` (fallback 체인) | **현재 사용 중** |
| v1 | 2026-03-22 | `evaluate_script`로 `document.execCommand('insertText', false, text)` 입력 — deprecated API이나 contenteditable에서 가장 안정적 | **현재 사용 중** |

### 스트리밍 완료 감지 — Stop 버튼

| 버전 | 날짜 | aria-label 값 | 상태 |
|------|------|--------------|------|
| v2 | 2026-03-25 | `"Stop response"` (EN ✅), `"응답 중단"` (KO ✅), `"Stop Response"`, `"응답 중지"` (fallback) — 전부 셀렉터 체인에 포함 | **현재 사용 중** |

### 스트리밍 완료 감지 — polling 방식

| 버전 | 날짜 | 방법 | 상태 |
|------|------|------|------|
| v2 | 2026-03-24 | async Promise + setInterval(3초) — 단일 evaluate_script 호출로 완료 대기. TAB_LOST 감지 포함. 5분 타임아웃 | **현재 사용 중 (preferred)** |
| v1 | 2026-03-22 | evaluate_script 반복 호출 (3~5초 간격) | **fallback** — async 미지원 환경 대비 |

### 스트리밍 완료 감지 — data-is-streaming

| 버전 | 날짜 | 셀렉터/방법 | 상태 |
|------|------|------------|------|
| v1 | 2026-03-22 | `[data-is-streaming="true"]` | **현재 사용 중** |

### 마지막 어시스턴트 응답 추출

| 버전 | 날짜 | 셀렉터/방법 | 상태 |
|------|------|------------|------|
| v1 | 2026-03-22 | `[data-testid="user-message"]`에서 부모 최대 10단계 탐색 → `nextElementSibling.innerText` | **현재 사용 중** |

### 추출 전 스크롤 (scroll-to-bottom)

| 버전 | 날짜 | 방법 | 상태 |
|------|------|------|------|
| v1 | 2026-03-24 | `[data-testid="user-message"]`에서 부모를 올라가며 첫 scrollable ancestor 탐색 (`scrollHeight > clientHeight + 100`) | **현재 사용 중 (권장)** |

### 사용자 메시지 식별

| 버전 | 날짜 | 셀렉터/방법 | 상태 |
|------|------|------------|------|
| v1 | 2026-03-22 | `[data-testid="user-message"]` | **현재 사용 중** |

### 파일 첨부

| 버전 | 날짜 | 방법 | 상태 |
|------|------|------|------|
| v3 | 2026-03-23 | clip-files 스크립트 → 클립보드 복사 → `evaluate_script` 포커스 → `press_key` Ctrl+V. uid 불필요. 컨텍스트 소모 0 | **현재 사용 중** |
...
| 버전 | 날짜 | 방법 | 상태 |
|------|------|------|------|
| v2 | 2026-03-23 | `evaluate_script`로 DOM에서 파일명 텍스트 검색 (~100 토큰). 스크린샷 불필요 | **현재 사용 중** |
| v1 | 2026-03-22 | `take_screenshot`으로 시각 확인 (~3,000 토큰 at 1440p) | **폐기** — DOM 탐색으로 대체, 토큰 30배 절약 |
