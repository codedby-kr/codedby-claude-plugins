...
## Input Parsing
- Extract the work objective, targets, and constraints from the natural language in `$ARGUMENTS`
...
- Verify that `$ARGUMENTS` contains enough information to identify work targets (files, directories, topics)
- If ambiguous or unclear → ask the user to clarify → wait
...
Analyze the user's message and explore related code/files to construct work items.

### Create _progress.md
Create `_progress.md` in `{project-root}/docs/{task-name}/`. If it already exists, read it and resume. If any item is marked as interrupted, check session memory first.
...
## Info
- Original instruction: (user's message verbatim)
...
## Key Constraints (extracted from user instruction)
- (constraints the user explicitly stated)
...
- Send web Claude the current item's work context (**file list required** + scope + purpose)
...
## Rules
- Commits are not managed by this command. If the user asks, execute it.
