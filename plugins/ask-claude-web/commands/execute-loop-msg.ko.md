---
argument-hint: '작업 지시사항 [웹 클로드 검증] [항목마다 커밋] [컨텍스트 N%까지]'
---
사용자의 메시지를 분석하여 작업 항목을 구성하고 순서대로 반복 실행한다. 사용자가 `$ARGUMENTS`에 작업 지시사항을 입력한다.
...
## 입력 파싱
- `$ARGUMENTS`의 자연어에서 작업 목적, 대상, 제약 조건을 파악
...
### 작업 대상 확인 (필수)
- `$ARGUMENTS`에서 작업 대상(파일, 디렉토리, 주제)이 파악 가능한지 확인
- 모호하거나 대상이 불분명하면 → 사용자에게 구체화 요청 → 대기
...
사용자의 메시지를 분석하고, 관련 코드/파일을 탐색하여 작업 항목을 구성한다.

### _progress.md 생성
프로젝트 루트/docs/{작업명} 에 `_progress.md`를 생성한다. 이미 있으면 읽고 이어서 진행.
...
- 웹 클로드에게 현재 항목의 작업 맥락(**대상 파일 목록 필수** + 범위 + 목적)을 전송
